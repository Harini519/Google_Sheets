...
document.addEventListener("DOMContentLoaded", () => {
  ...
  tableBody.addEventListener("mousedown", (e) => {
      if (e.target.tagName === "INPUT") {
          isDragging = true;
          startCell = e.target;
          startCell.classList.add("drag-start");
      }
  });
  ...
  tableBody.addEventListener("mouseup", (e) => {
      if (isDragging) {
          ...
          document.querySelectorAll(".drag-highlight").forEach(cell => {
              cell.value = startCell.value; // Example behavior: copy starting cell's value
              cell.classList.remove("drag-highlight");
          });
          startCell.classList.remove("drag-start");
      }
  });

  document.addEventListener("mouseleave", () => {
      isDragging = false; // Reset drag state on mouse exit
  });
});
